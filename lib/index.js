...
exports.default = function (projectPath, contractsPath, docusaurusPath, repositoryRoot, excludePaths) {
  ...
  (0, _checkPathExists2.default)(repositoryRoot);
...
function parseProject(solidityCompilerPath, solidityCompilerExtraArgs, contractsPath) {
  var commandOutput = _shelljs2.default.exec(['' + solidityCompilerPath, '  --pretty-json', '  --allow-paths ' + contractsPath, '  --combined-json ' + COMBINED_JSON_OPTIONS.join(','), '  ' + solidityCompilerExtraArgs, '  $(find ' + contractsPath + ' -type f -name "*.sol")'].join(' '), { silent: true, maxBuffer: 102400000 });
